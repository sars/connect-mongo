  ...
  function MongoStore(options, callback) {
    ...
    if (options.mongoose_connection){
      ...
      if (Array.isArray(options.mongoose_connection.hosts)) {
          var servers = [];
          for(var i = 0; i < options.mongoose_connection.hosts.length; i++) {
              servers.push(new mongo.Server(options.mongoose_connection.hosts[i].host,
                                      options.mongoose_connection.hosts[i].port,
                                      options.mongoose_connection.db.serverConfig.options));
          }
          var set = new mongo.ReplSetServers(servers, { rs_name: options.rs_name });
          this.db = new mongo.Db(options.mongoose_connection.db.databaseName, set,
